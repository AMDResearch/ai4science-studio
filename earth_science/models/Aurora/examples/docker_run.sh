#!/usr/bin/env bash
# docker_run.sh — Build the PyTorch weather container and launch an interactive session
#
# USER EDITS REQUIRED:
...
#
# NOTE: Aurora uses a PyTorch image (pytorchweather:latest), separate from
#       the JAX image used by PanguWeather and GenCast.
...
IMAGE_NAME="pytorchweather:latest"
...
# --- Build PyTorch image if not present ---
if ! docker image inspect "$IMAGE_NAME" &>/dev/null; then
    echo "Building $IMAGE_NAME ..."
    docker build -t "$IMAGE_NAME" -f "$RECIPE_DIR/pytorch.dockerfile" "$RECIPE_DIR"
else
...
mkdir -p "$CACHE_DIR" "$MODEL_DIR/examples/predictions"

echo ""
echo "Launching $IMAGE_NAME for Aurora ..."
