# Earth science model landmines (StormCast, ORBIT-2, weather stack)
...
- **xformers.components shim:** ORBIT-2 imports `xformers.components.attention.core.scaled_dot_product_attention`. This subpackage was removed in xformers ≥0.0.28. Write a three-file shim after install:
  ...
  ```
- **Distributed launch:** `srun --mpi=pmix apptainer exec ... --env HOSTNAME="$MASTER_ADDR"` — see [studio `reference/rocm.md`](../ai4science-studio/reference/rocm.md) for the full pattern. Confirmed working for 1-GPU and 8-GPU.
...
**Correct pattern — NFS staging + strip + copy:** see [studio `reference/rocm.md`](../ai4science-studio/reference/rocm.md). Use `--extra-index-url https://download.pytorch.org/whl/<ROCM_WHL_TAG>` (no constraint); ROCm torch has no `nvidia-*` CUDA co-package deps so the entire CUDA chain is never resolved.
...
- **StormCast no-overlay path**: uses the install-to-tmp + bind-mount pattern (see [studio `reference/rocm.md`](../ai4science-studio/reference/rocm.md)). Packages go to `/opt/stormcast-pkgs` via `--target` + bind-mount. `PYTHONPATH` must include this path in the inference `bash -c` block.
