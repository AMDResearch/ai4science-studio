...
#   ${PERF_TOOLS_DIR}/perf-inspect/    (created by the launcher subagent)
#   ${PERF_TOOLS_DIR}/victoriametrics/   (created by the launcher subagent)
#
...
# Cluster-specific: set from .cluster-config.yaml perf_tools.dir.
...
if [[ "$OMNISTAT_KERNEL_TRACE" == "1" ]]; then
  if [[ ! -f "$OMNISTAT_TRACE_LIB" ]]; then
    ...
    echo "             cd ${PERF_TOOLS_DIR}/omnistat-src && \\" >&2
